...
test.describe('AIV Login and Create MySQL', () => {
  test('Login, Navigate, and Create MySQL', async ({ page }) => {
    ...
    try {
      ...
      await page1.locator('input[name="name"]').fill('dynamic-Parameter_Testing');
      ...
      await page1.getByRole('option', { name: 'label_list_box' }).click();
      ...
      await page2.getByRole('option', { name: 'dynamic-Parameter_Testing' }).click();
      ...
      await page2.getByRole('textbox', { name: 'Dataset Name' }).fill('Category_Metal1');
