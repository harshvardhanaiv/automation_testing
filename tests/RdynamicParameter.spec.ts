...
test.describe('AIV Login and Create MySQL', () => {
  test('Login, Navigate, and Create MySQL', async ({ page }) => {
    ...
    try {
      ...
      await page1.locator('input[name="name"]').fill('Parameter4_Testing');
      ...
      await page1.getByRole('option', { name: 'label_Radio_Button' }).click();
      ...
      await page1.getByRole('textbox', { name: 'Search by name or type' }).fill('met');
      ...
      await page2.getByRole('option', { name: 'Parameter4_Testing' }).click();
      ...
      await page2.locator('div').filter({ hasText: /^Electrical$/ }).locator('div').nth(2).click();
      await page2.getByRole('button', { name: 'Submit' }).click();
      await page2.getByRole('textbox', { name: 'Dataset Name' }).click();
      await page2.getByRole('textbox', { name: 'Dataset Name' }).fill('Category_Metal');
